...
export default function IncidentList() {
  ...
  useEffect(() => {
    const fetchIncidents = async () => {
      const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/incidents?resolved=false`);
...
useEffect(() => {
  const fetchResolved = async () => {
    const res = await fetch(`${process.env.NEXT_PUBLIC_API_URL}/incidents?resolved=true`);
    const data = await res.json();
  ...
  return (
    <div className="w-2/5 h-[450px] bg-[#131313] rounded-lg p-4 overflow-hidden relative">
        <div className="flex items-center justify-between mb-4 px-1">
  {/* Left: Unresolved */}
  <div className="flex items-center gap-2 text-white font-semibold">
    <Image src={unresolvedIncidents} alt="Unresolved Incidents" width={24} height={24} />
  ...
  <div className="flex items-center   text-xs">
    {/* three icons badges */}
      ...
      <Image src={BadgeBase} alt="Badge" width={16} height={16} />
    
    
      <Image src={BadgeBase1} alt="Badge" className='-ml-1 'width={16} height={16} />
    
      <Image src={BadgeBase2} alt="Badge" className='-ml-1 mr-2' width={16} height={16} />
 

    <div className="bg-zinc-950 text-zinc-50 px-2 py-0 rounded-full border border-zinc-700 flex items-center gap-1">
    ...
    <div className="overflow-y-auto scrollbar-none max-h-[430px]">
      {incidents.map((incident) => (
        <IncidentCard key={incident.id} incident={incident} onResolve={handleResolve} />
      ))}

        {/* If no incidents, show message */}
      {incidents.length === 0 && (
